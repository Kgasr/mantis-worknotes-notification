...
from src.config.mantis_config import MantisConfig
...
def is_recently_updated(updated_at, timestamp_from):
    try:
...
def is_recently_created(created_at, updated_at, timestamp_from = None):
    try:
        formatted_created_at = datetime.fromisoformat(created_at)
        formatted_created_at = datetime.strftime(formatted_created_at, "%Y-%m-%d %H:%M")
        formatted_updated_at = datetime.fromisoformat(updated_at)
        formatted_updated_at = datetime.strftime(formatted_updated_at, "%Y-%m-%d %H:%M")
        if formatted_created_at == formatted_updated_at:        # new ticket and no new work notes added
            return True
        else :                                                  # New ticket and new work notes added as well
            created_at = datetime.fromisoformat(created_at)
            created_at = datetime.strftime(created_at, "%Y-%m-%d %H:%M:%S")
            # Check if the issue was created after timestamp_from
            return created_at > timestamp_from
    except Exception as e:
...
class MantisHandler:
    ...
    def __fetch_updated_issues_and_worknotes_since_timestamp(self, issues, timestamp_from):
        ...
        updated_notes = []
        for issue in issues:
            if is_recently_updated(issue['updated_at'], timestamp_from):
                issue_data = extract_fields(issue, self.__mantis_config.issue_fields, "Issue ")
                if is_recently_created(issue['created_at'], issue['updated_at'], timestamp_from) :
                ...
                for note in notes:
                    if is_recently_updated(note['updated_at'], timestamp_from):
                        ...
                        note_data.pop("Issue Attachments Path", None)  # Remove key if it exists
    ...
    def fetch_recently_updated_issues(self, minutes=1):
        try:
            ...
            updated_issues_ids_list = self.__mysql_handler.get_updated_issues_ids_list_between_range(start_time, end_time)
